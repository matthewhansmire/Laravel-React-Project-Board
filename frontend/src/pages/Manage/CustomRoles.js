...
class CustomRoles extends Component {
  constructor(props) {
    super(props);
    this.state = {
      customRoles: [
        { role: "Owner", badge: "default" },
        { role: "Admin", badge: "default" },
        { role: "Normal user", badge: "" },
      ]
    };
  }
    ...
    return (
      <React.Fragment>
        ...
        <CardHeader className="px-0 py-2 font-size-16 d-flex align-items-center justify-content-between" id="headingTwo">
          <Button className="bg-dark" onClick={() => this.props.showRightBar('customRole')}><i className="bx bx-plus font-size-16 align-middle mr-2"></i> Add</Button>
          <i className={`${Icons.help} font-size-18`} style={{ cursor: "pointer" }} onClick={() => { }}></i>
        </CardHeader>
        <CardBody className="bg-white">
          {
            this.state.customRoles.map((each) => {
              return (
                <>
                  <Row>
                    <Col className="col-12 d-flex align-items-center">
                      <span className="font-size-14 ml-2">{each.role}</span>
                      {
                        each.badge &&
