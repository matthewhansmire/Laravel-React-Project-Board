...
class ProjectCategories extends Component {
  constructor(props) {
    ...
    this.state = {
      projectCategories: [
        { category: "Uncategorized", badge: "default" },
        { category: "Categorized", badge: "" },
      ]
    };
  }
    ...
    return (
      <React.Fragment>
        ...
        <CardHeader className="px-0 py-2 font-size-16 d-flex align-items-center justify-content-between" id="headingTwo">
          <Button className="bg-dark" onClick={() => this.props.showRightBar('projectCategory')}><i className="bx bx-plus font-size-16 align-middle mr-2"></i> Add</Button>
          <i className={`${Icons.help} font-size-18`} style={{ cursor: "pointer" }} onClick={() => { }}></i>
        </CardHeader>
        <CardBody className="bg-white">
          {
            this.state.projectCategories.map((each) => {
              return (
                <>
                  <Row>
                    <Col className="col-12 d-flex align-items-center">
                      <span className="font-size-14 ml-2">{each.category}</span>
                      {
                        each.badge &&
...
export default connect(mapStateToProps, {
  showRightBar,
  hideRightBar
})(withNamespaces()(ProjectCategories));
